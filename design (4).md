...
**Architecture Diagram**: The complete AWS architecture diagram with official service icons is available at the project root as `vericrop_finbridge_architecture.png`. This diagram shows all 25+ AWS services and their interactions across three logical zones:

1. **Edge/Field Zone** - AWS IoT Greengrass v2, Mobile App, Voice Interface (Lex/Polly), Transfer Learning Model
2. **Logic & Analysis Zone** - Step Functions Express, Forensic Validators (Solar Azimuth, Weather, Rekognition, AI), Amazon A2I HITL, Core Lambda Functions, DynamoDB
3. **Trust & Ledger Zone** - Amazon QLDB, Managed Blockchain (Hyperledger Fabric), Bridge Loan Engine, UPI Payment Gateway

**Key Data Flow**: Farmer → Mobile (Voice) → Greengrass → AppSync → SQS → Step Functions → Parallel Forensic Validation (60s) → Lambda Core → QLDB/Blockchain → Bridge Loan → UPI Payment (60s total) → Farmer

For a landscape version optimized for presentations (16:9 aspect ratio), see `vericrop_finbridge_architecture_16x9.png`.
